...
import suggestedProfilesFixture from '../../fixtures/suggested-profile';
...
describe('<Dashboard />', () => {
  ...
  it('renders the dashboard with a user profile and follows a user from the suggested profile', async () => {
    await act(async () => {
      ...
      const { getByText, getByAltText, getByTitle, getAllByText, getAllByAltText, getByTestId } = render(
        <Router>
          ...
          >
            <UserContext.Provider
              value={{
                user: {
                  uid: 'meoqrKaMqsecY3rM6T7KnR8duGn1',
                  displayName: 'pr1mus'
                }
              }}
            >
      ...
      await waitFor(() => {
        ...
        //expect(getByAltText('default profile')).toBeTruthy();
